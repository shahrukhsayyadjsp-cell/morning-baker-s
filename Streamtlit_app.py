...
if choice == "Customer Management":
    ...
    with st.form("Add Customer"):
        ...
        pav_type = st.text_input("Pav Type (पाव का प्रकार - e.g., Ladi Pav, Special)")
        rate = st.number_input("Rate per Pav (एक充 पाव की कीमत)", min_value=0.0, value=20.0, step=0.5)
    ...
    if not customers:
        ...
        with st.form("daily_form"):
            ...
            for cust in customers:
                cust_id, cust_name, cust_pav, cust_rate = cust
                col1, col2, col3, col4 = st.columns()
                with col1:
                    st.write(f"**{cust_name}** ({cust_pav} - ₹{cust_rate})")
                with col2:
                    qty = st.number_input(f"Qty", min_value=0, value=0, key=f"qty_{cust_id}_{date_today}")
                with col3:
                    total_p = qty * cust_rate
                    st.write(f"Total: ₹{total_p}")
                with col4:
                    status = st.selectbox(f"Payment", ["Udhar", "Cash"], key=f"status_{cust_id}_{date_today}")
                
                if qty > 0:
                    entries.append((date_today, id, qty, total_p, status))
                st.write("---")
...
# --- 3. RECEIVE PAYMENT ---
    ...
    cust_dict = {cust[1]: cust[0] for cust in customers}
        ...
        if st.button("💵 Record Payment"):
            if amount_paid > 0:
                c.execute("INSERT INTO payments (date, customer_id, amount_paid) VALUES (?, ?, ?)", 
                          (date_pay, cust_dict[selected_cust], amount_paid))
                conn.commit()
                st.success(f"💸 {selected_cust} के ₹{amount_paid} जमा कर लिए गए हैं!")
            else:
...
# --- 4. UDHAR REPORT & HISTORY ---
    ...
    st.subheader("📜 Full Delivery History (पिछला सारा记录)")
    query_history = """
